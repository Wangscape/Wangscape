...
{
    EXPECT_EQ(5., abs(x)->getValue(-5., 0, 0));
    EXPECT_EQ(4., abs(x)->getValue(4., 0, 0));
    EXPECT_EQ(3., abs(y)->getValue(-5., 3., 0));
    EXPECT_EQ(2., abs(y)->getValue(4., -2., 0));
}

TEST_F(TestReseedable, TestClampReseedable)
{
    EXPECT_EQ(-1., clamp(x, -1., 1.)->getValue(-5., 0, 0));
    EXPECT_EQ(1., clamp(y, -1., 1.)->getValue(-1., 3., 0));
    EXPECT_EQ(0., clamp(x, -5., 5.)->getValue(0., 3., 0));
    EXPECT_EQ(-10., clamp(y, -10, -10)->getValue(4., -2., 0));
}

TEST_F(TestReseedable, TestPowReseedableDouble)
{
    EXPECT_NEAR(-8., pow(x, 3)->getValue(-2., 0, 0), 0.0001);
    EXPECT_EQ(0.04, pow(x, -2)->getValue(5., 0, 0));
    EXPECT_NEAR(3., pow(y, 0.5)->getValue(-5., 9., 0), 0.0001);
    EXPECT_EQ(1., pow(y, 0)->getValue(4., -2., 5.));
}

TEST_F(TestReseedable, TestPowReseedableReseedable)
{
    EXPECT_EQ(4., pow(x, x)->getValue(2., 0, 0));
    EXPECT_EQ(-32., pow(x, y)->getValue(-2., 5, 0));
    EXPECT_NEAR(-1953125., pow(y, x)->getValue(9., -5., 0), 0.01);
    EXPECT_EQ(1. / 256., pow(y, y)->getValue(4., -4., 5.));
}

TEST_F(TestReseedable, TestPowDoubleReseedable)
{
    EXPECT_NEAR(1./9., pow(3., x)->getValue(-2., 0, 0), 0.0001);
    EXPECT_EQ(-32., pow(-2., x)->getValue(5., 0, 0));
    EXPECT_NEAR(0.25, pow(0.5, y)->getValue(-5., 2., 0), 0.0001);
    EXPECT_EQ(0., pow(0., y)->getValue(4., 2., 5.));
}

TEST_F(TestReseedable, TestInvReseedable)
{
    EXPECT_EQ(16., invert(x)->getValue(-16., 0, 0));
    EXPECT_EQ(-32., invert(x)->getValue(32., 0, 0));
    EXPECT_EQ(3., invert(y)->getValue(-5., -3., 0));
    EXPECT_EQ(-1., invert(y)->getValue(4., 1., 5.));
}

TEST_F(TestReseedable, TestScaleBiasReseedable)
{
    EXPECT_EQ(-155, scaleBias(x, 10, 5)->getValue(-16., 0, 0));
    EXPECT_EQ(-158, scaleBias(x, -5, 2)->getValue(32., 0, 0));
    EXPECT_EQ(1.5, scaleBias(y, 0.5, 3)->getValue(-5., -3., 0));
    EXPECT_EQ(1.5, scaleBias(y, 1, 0.5)->getValue(4., 1., 5.));
}
...
{
    EXPECT_EQ(-11., (5. + x)->getValue(-16., 0, 0));
    EXPECT_EQ(-6., ((-3) + y)->getValue(-5., -3., 0));
}
...
{
    EXPECT_EQ(-3., max(x, y)->getValue(-5., -3., 0));
    EXPECT_EQ(4., max(y, x)->getValue(4., 1., 5.));
    EXPECT_EQ(-3., max(y, x)->getValue(-5., -3., 0));
    EXPECT_EQ(4., max(x, y)->getValue(4., 1., 5.));
}

TEST_F(TestReseedable, TestMaxReseedableDouble)
{
    EXPECT_EQ(4., max(x, 4.)->getValue(-5., -3., 0));
    EXPECT_EQ(1., max(y, 0.)->getValue(4., 1., 5.));
}

TEST_F(TestReseedable, TestMaxDoubleReseedable)
{
    EXPECT_EQ(1., max(0., y)->getValue(4., 1., 5.));
    EXPECT_EQ(4., max(4., x)->getValue(-5., -3., 0));
}
TEST_F(TestReseedable, TestMinReseedableReseedable)
{
    EXPECT_EQ(-5., min(x, y)->getValue(-5., -3., 0));
    EXPECT_EQ(1., min(y, x)->getValue(4., 1., 5.));
    EXPECT_EQ(-5., min(y, x)->getValue(-5., -3., 0));
    EXPECT_EQ(1., min(x, y)->getValue(4., 1., 5.));
}

TEST_F(TestReseedable, TestMinReseedableDouble)
{
    EXPECT_EQ(-5., min(x, 4)->getValue(-5., -3., 0));
    EXPECT_EQ(0., min(y, 0)->getValue(4., 1., 5.));
}

TEST_F(TestReseedable, TestMinDoubleReseedable)
{
    EXPECT_EQ(0., min(0, y)->getValue(4., 1., 5.));
    EXPECT_EQ(-5., min(4, x)->getValue(-5., -3., 0));
}
...
{
    EXPECT_EQ(21., (5. - x)->getValue(-16., 0, 0));
    EXPECT_EQ(-1., ((-4) - y)->getValue(-5., -3., 0));
}
...
{
    EXPECT_EQ(-80., (5. * x)->getValue(-16., 0, 0));
    EXPECT_EQ(12., ((-4) * y)->getValue(-5., -3., 0));
}
...
{
    EXPECT_EQ(0.75, blend(c05, x, y)->getValue(0., 1., 5.));
    EXPECT_EQ(-5, blend(c05, y, x)->getValue(-1, -17, 5.));
}
TEST_F(TestReseedable, TestSelectReseedable)
{
    EXPECT_EQ(-17, select(c05, x, y)->getValue(-1, -17, 5.));
    EXPECT_EQ(0., select(c05, y, x)->getValue(0., 1., 5.));
}
TEST_F(TestReseedable, TestTranslateReseedableReseedable)
{
    auto translate_p = translate((x - (y * 2)), x, y, c05);
    EXPECT_EQ(0., translate_p->getValue(0, 0, 304958.));
    EXPECT_EQ(0., translate_p->getValue(2, 1, 30458.));
    EXPECT_EQ(-16., translate_p->getValue(-2, 3, 30458.));
}
TEST_F(TestReseedable, TestTranslateReseedableDouble)
{
    EXPECT_EQ(3., translate(x, 1, 10, 100)->getValue(2, 20, 200));
    EXPECT_EQ(-10., translate(y, 1, 10, 100)->getValue(2, -20, 200));
}
TEST_F(TestReseedable, TestRotatePointReseedable)
{
    EXPECT_EQ(20., rotatePoint(x, 0,0,90)->getValue(2, 20, 200));
    EXPECT_EQ(-30, rotatePoint(y, 0,0,180)->getValue(2, 30, 200));

}
TEST_F(TestReseedable, TestScalePointReseedable)
{
    EXPECT_EQ(6., scalePoint(x, 3, 30, 300)->getValue(2, 20, 200));
    EXPECT_EQ(200., scalePoint(y, -1, -10, -100)->getValue(2, -20, 200));

}
TEST_F(TestReseedable, TestTurbulenceReseedable)
{
    auto turbulence_p = turbulence(c05, 2., 2., 3, 0);
    EXPECT_EQ(0.5, turbulence_p->getValue(346980., 0.63, 696.346));
}
...
TEST_F(TestReseedable, TestTerraceReseedableIter)
{
    ...
    EXPECT_NEAR(0., terrace_p->getValue(0., 239478, -23984), 0.00001);
    EXPECT_NEAR(0.5, terrace_p->getValue(0.5, -239478, -23984), 0.00001);
    EXPECT_NEAR(1., terrace_p->getValue(1., -239478, 23984), 0.00001);

    EXPECT_GT(0.25, terrace_p->getValue(0.25, 3957, -9723));
    EXPECT_GT(0.75, terrace_p->getValue(0.75, -3957, 9723));
}
...
TEST_F(TestReseedable, TestTerraceReseedableInt)
{
    ...
    EXPECT_NEAR(-1., terrace_p->getValue(239478, -1, -23984), 0.00001);
    EXPECT_NEAR(-1. / 3., terrace_p->getValue(-239478, -1. / 3., -23984), 0.00001);
    EXPECT_NEAR(1. / 3., terrace_p->getValue(-239478, 1. / 3., 23984), 0.00001);
    EXPECT_NEAR(1., terrace_p->getValue(239478, 1., 23984), 0.00001);

    EXPECT_LT(-2. / 3., terrace_p->getValue(3957, -2. / 3., -9723));
    EXPECT_LT(0., terrace_p->getValue(-3957, 0., -9723));
    EXPECT_LT(2. / 3., terrace_p->getValue(-3957, 2. / 3., 9723));
}
...
TEST_F(TestReseedable, TestCurveReseedable)
{
    ...
    EXPECT_EQ(-1., curve_p->getValue(-1., 239478, -23984));
    EXPECT_EQ(1., curve_p->getValue(-0.5, 239478, -23984));
    EXPECT_EQ(-1., curve_p->getValue(0., 239478, -23984));
    EXPECT_EQ(1., curve_p->getValue(0.5, 239478, -23984));
    EXPECT_EQ(-1., curve_p->getValue(1., 239478, -23984));

    EXPECT_LT(-0.1, curve_p->getValue(-0.75, 239478, -23984));
    EXPECT_GT(1, curve_p->getValue(-0.75, 239478, -23984));
    EXPECT_LT(-0.1, curve_p->getValue(-0.25, 239478, -23984));
    EXPECT_GT(1, curve_p->getValue(-0.25, 239478, -23984));
    EXPECT_LT(-0.1, curve_p->getValue(0.25, 239478, -23984));
    EXPECT_GT(1, curve_p->getValue(0.25, 239478, -23984));
    EXPECT_LT(-0.1, curve_p->getValue(0.75, 239478, -23984));
    EXPECT_GT(1, curve_p->getValue(0.75, 239478, -23984));
